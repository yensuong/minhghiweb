...
function responsive_slider() {
	...
	if ( $slides->have_posts() ) :
		...
		$slider = '<div class="responsive-slider flexslider header_slider">';
			...
			while ( $slides->have_posts() ) : $slides->the_post();
				...
				$slider .= '<li>';
					...
					$slider .= '<div id="slide-' . get_the_ID() . '" class="slide">';
						...
						$sub_title = get_post_meta( $post->ID, "_slide_sub_title", true );
						$slider .= '<div class="slide-title"><div class="container"><h2 class="slide-main-title"><a href="' . get_post_meta( $post->ID, "_slide_link_url", true ) . '" title="' . the_title_attribute ( array( 'echo' => 0 ) ) . '">' . the_title_attribute ( array( 'echo' => 0 ) ) . '</a></h2>'.(!empty($sub_title) ? '<div class="slide-sub-title">'.$sub_title.'</div>' : '').'</div></div>';
...
function responsive_slider_metabox_1() {
	...
	$slide_sub_title = get_post_meta( $post->ID, '_slide_sub_title', true ); ?>
	...
	<p>Sub title: <br /><textarea name="slide_sub_title" style="width: 100%;"><?php echo $slide_sub_title; ?></textarea></p>
...
function responsive_slider_save_meta( $post_id, $post ) {
    ...
    if ( isset( $_POST['slide_sub_title'] ) ) {
        update_post_meta( $post_id, '_slide_sub_title', strip_tags( $_POST['slide_sub_title'] ) );
    }
}
